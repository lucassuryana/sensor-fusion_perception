...
def detect_objects(input_bev_maps, model, configs):
    ...
    if len(detections) > 0:
        ...
        width = configs.bev_width
        for det in detections:
            ## step 3 : perform the conversion using the limits for x, y and z set in the configs structure
            # x, y, z, h, w, l, yaw
            _, x_bev, y_bev, z, h, w_bev, l_bev, yaw = det
            x = y_bev / height * (lim_x[1] - lim_x[0])
            y = x_bev / width * (lim_y[1] - lim_y[0]) - (lim_y[1] - lim_y[0]) / 2.0
            w = w_bev / width * (lim_y[1] - lim_y[0])
            l = l_bev / height * (lim_x[1] - lim_x[0])
            ## step 4 : append the current object to the 'objects' array
            if ((x >= lim_x[0]) and (x <= lim_x[1])
                    and (y >= lim_y[0]) and (y <= lim_y[1])
                    and (z >= lim_z[0]) and (z <= lim_z[1])):
