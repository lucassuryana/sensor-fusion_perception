...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            label_corners = tools.compute_box_corners(label.box.center_x, label.box.center_y, label.box.width,
                                                      label.box.length, label.box.heading)
...
def compute_performance_stats(det_performance_all):
    ...
    true_positives = sum([item[2][1] for item in det_performance_all])
    false_negatives = sum([item[2][2] for item in det_performance_all])
    false_positives = sum([item[2][3] for item in det_performance_all])
